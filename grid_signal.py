...
def main():
    ...
    def signal_handler(signal: TradingSignal):
        """處理交易訊號的回調函數"""
        print(f"收到訊號: {signal.symbol} {signal.side.value} @ {signal.price} 數量:{signal.size} 類型:{signal.signal_type}")
        # 這裡可以調用 client.py 中的實際交易函數
    
    # 創建BTC合約網格交易訊號生成器（包含停損設置）
    signal_generator = GridSignalGenerator(
        ticker="BTCUSDT",
        current_price=42500,       # 當前價格 42500 USDT
        direction=Direction.BOTH,  # 雙向網格
        upper_bound=45000,         # 上界 45000 USDT
        lower_bound=40000,         # 下界 40000 USDT  
        grid_levels=6,             # 6個網格
        total_amount=1000,         # 總投入 1000 USDT
        stop_bot_price=38000,      # 下界停損 38000 USDT
        stop_top_price=47000,      # 上界停損 47000 USDT
        signal_callback=signal_handler
    )
    
    # 設置初始網格
    signal_generator.setup_initial_grid()
    
    # 獲取狀態
    signal_generator.get_status()
    
    # 模擬訂單成交
    print("\n=== 模擬訂單成交 ===")
    # 模擬一個買入訂單成交（假設在 42000 價格成交）
    ...
    signal_generator.on_order_filled(test_signal)
    
    signal_generator.get_status()
    
    # 測試手動停止功能
    print("\n=== 測試手動停止 ===")
    signal_generator.stop_by_signal()  # 模擬接收API停止訊號
    ...
    # 模擬觸發停損
    print("\n=== 模擬觸發下界停損 ===")
    signal_generator.check_stop_conditions(37500)  # 低於停損價格 38000
    ...
    print("\n=== API控制示例 ===")
    print("未來可通過以下方式遠程控制：")
    print("POST /api/grid/stop - 停止網格交易訊號生成")
    print("POST /api/grid/start - 重新啟動網格交易訊號生成") 
    print("GET /api/grid/status - 獲取網格狀態")
